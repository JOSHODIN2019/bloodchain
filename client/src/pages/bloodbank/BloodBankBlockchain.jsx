import { useState, useEffect } from 'react'
import { useAuth } from '@/contexts/AuthContext'
import { bloodbankService } from '@/services/bloodbankService'
import { Spinner, Alert, Badge } from '@/components/ui'
import VerifyButton from '@/components/VerifyButton'

function truncHash(h, n = 12) {
  if (!h) return '—'
  return `${h.slice(0, n)}…${h.slice(-6)}`
}
function fmtDate(d) {
  if (!d) return '—'
  return new Date(d).toLocaleDateString('en-NG', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })
}

export default function BloodBankBlockchain() {
  const { user }              = useAuth()
  const [donations, setDonations] = useState([])
  const [requests,  setRequests]  = useState([])
  const [loading,   setLoading]   = useState(true)
  const [error,     setError]     = useState('')
  const [tab,       setTab]       = useState('donations')

  useEffect(() => {
    bloodbankService.getBlockchain()
      .then(d => { setDonations(d.donations); setRequests(d.requests) })
      .catch(() => setError('Could not load blockchain records.'))
      .finally(() => setLoading(false))
  }, [])

  const total = donations.length + requests.length

  if (loading) return <div className="flex items-center justify-center h-64"><Spinner size="lg" /></div>

  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">

      <div>
        <h1 className="text-xl font-bold text-neutral-900">Blockchain Trail</h1>
        <p className="text-sm text-neutral-500 mt-0.5">On-chain records for your blood bank on Ethereum Sepolia</p>
      </div>

      {error && <Alert variant="warning">{error}</Alert>}

      {/* Network banner */}
      <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-5 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center flex-shrink-0">
            <ChainIcon />
          </div>
          <div>
            <p className="text-white font-semibold text-sm">Ethereum Sepolia Testnet</p>
            <p className="text-slate-400 text-xs mt-0.5">{user?.organizationName || user?.fullName}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-white/10 border border-white/10 rounded-xl px-4 py-2 text-center">
            <p className="text-white font-bold text-lg leading-none">{total}</p>
            <p className="text-slate-400 text-[10px] mt-1">On-Chain</p>
          </div>
          <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            Live
          </span>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 p-1 bg-neutral-100 rounded-xl w-fit">
        {[
          { key: 'donations', label: `Donations (${donations.length})` },
          { key: 'requests',  label: `Requests (${requests.length})`  },
        ].map(({ key, label }) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
              tab === key ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'
            }`}
          >
            {label}
          </button>
        ))}
      </div>

      {tab === 'donations' && (
        <TxList
          items={donations}
          emptyMsg="No on-chain donation records yet. When donors' donations are confirmed, transaction hashes will appear here."
          renderItem={d => (
            <TxCard key={d._id}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-red-50 flex items-center justify-center flex-shrink-0">
                    <BloodIcon />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">{d.donorId?.fullName || '—'}</p>
                    <p className="text-xs text-neutral-400 font-mono">{d.donorId?.userId} · {fmtDate(d.confirmedAt || d.createdAt)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-red-600">{d.bloodType}</span>
                  <span className="text-xs text-neutral-500">{d.units} unit{d.units !== 1 ? 's' : ''}</span>
                  <Badge variant="success" size="sm">Confirmed</Badge>
                </div>
              </div>
              <HashSection txHash={d.txHash} blockNumber={d.blockNumber} blockchainHash={d.blockchainHash} />
              {d.blockNumber && (
                <div className="mt-3 pt-3 border-t border-neutral-100">
                  <VerifyButton id={d._id} type="donation" />
                </div>
              )}
            </TxCard>
          )}
        />
      )}

      {tab === 'requests' && (
        <TxList
          items={requests}
          emptyMsg="No on-chain request records yet. Fulfilled blood requests will appear here."
          renderItem={r => (
            <TxCard key={r._id}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-purple-50 flex items-center justify-center flex-shrink-0">
                    <RequestIcon />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">{r.hospitalId?.organizationName || r.hospitalId?.fullName || '—'}</p>
                    <p className="text-xs text-neutral-400 font-mono">{r.hospitalId?.userId} · {fmtDate(r.createdAt)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-red-600">{r.bloodType}</span>
                  <span className="text-xs text-neutral-500">{r.units} unit{r.units !== 1 ? 's' : ''}</span>
                  <Badge variant="success" size="sm">Fulfilled</Badge>
                </div>
              </div>
              <HashSection txHash={r.txHash} blockNumber={r.blockNumber} blockchainHash={r.blockchainHash} />
              {r.blockNumber && (
                <div className="mt-3 pt-3 border-t border-neutral-100">
                  <VerifyButton id={r._id} type="request" />
                </div>
              )}
            </TxCard>
          )}
        />
      )}

      <InfoNote text="Blockchain records are permanent and tamper-proof. Each confirmed donation and fulfilled request creates an immutable transaction on the Ethereum Sepolia network." />
    </div>
  )
}

function TxList({ items, emptyMsg, renderItem }) {
  if (items.length === 0) {
    return (
      <div className="bg-white border border-neutral-200 rounded-2xl p-10 flex flex-col items-center justify-center gap-4">
        <div className="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center">
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none" stroke="#94a3b8" strokeWidth="1.5" strokeLinecap="round">
            <path d="M7 7l7-4 7 4v10l-7 4-7-4V7z"/><path d="M14 3v18M7 7l7 4 7-4"/>
          </svg>
        </div>
        <div className="text-center">
          <p className="font-semibold text-neutral-700">No on-chain records yet</p>
          <p className="text-sm text-neutral-400 mt-1 max-w-xs">{emptyMsg}</p>
        </div>
      </div>
    )
  }
  return <div className="space-y-3">{items.map(renderItem)}</div>
}

function TxCard({ children }) {
  return <div className="bg-white border border-neutral-200 rounded-2xl p-5 hover:shadow-md transition-shadow">{children}</div>
}

function HashSection({ txHash, blockNumber, blockchainHash }) {
  return (
    <div className="mt-4 space-y-2">
      {txHash && <HashRow label="Transaction Hash" value={txHash} link={`https://sepolia.etherscan.io/tx/${txHash}`} />}
      {blockchainHash && <HashRow label="Data Hash" value={blockchainHash} />}
      {blockNumber && (
        <div className="flex items-center gap-3 px-3 py-2 bg-neutral-50 rounded-lg">
          <p className="text-xs text-neutral-400 font-medium w-36 flex-shrink-0">Block Number</p>
          <p className="text-xs font-mono text-neutral-700">{blockNumber.toLocaleString()}</p>
        </div>
      )}
      {!txHash && !blockNumber && (
        <div className="px-3 py-2 bg-amber-50 rounded-lg">
          <p className="text-xs text-amber-600 italic">Simulation mode — no on-chain hash yet. Configure blockchain credentials to write real transactions.</p>
        </div>
      )}
    </div>
  )
}

function HashRow({ label, value, link }) {
  const [copied, setCopied] = useState(false)
  const copy = () => { navigator.clipboard.writeText(value); setCopied(true); setTimeout(() => setCopied(false), 2000) }
  return (
    <div className="flex items-center gap-3 px-3 py-2 bg-neutral-50 rounded-lg">
      <p className="text-xs text-neutral-400 font-medium w-36 flex-shrink-0">{label}</p>
      <p className="text-xs font-mono text-neutral-700 flex-1 truncate">{value}</p>
      <div className="flex items-center gap-1 flex-shrink-0">
        <button onClick={copy} className="w-6 h-6 flex items-center justify-center rounded hover:bg-neutral-200 transition-colors" title="Copy">
          {copied
            ? <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="#10b981" strokeWidth="1.8" strokeLinecap="round"><path d="M1.5 5l2.5 2.5L8.5 2"/></svg>
            : <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round"><rect x="3.5" y="3.5" width="5" height="5" rx="0.8"/><path d="M1 6V1.8A.8.8 0 011.8 1H6"/></svg>
          }
        </button>
        {link && (
          <a href={link} target="_blank" rel="noreferrer" className="w-6 h-6 flex items-center justify-center rounded hover:bg-neutral-200 transition-colors text-neutral-400 hover:text-emerald-600 text-[10px] font-bold" title="View on Etherscan">↗</a>
        )}
      </div>
    </div>
  )
}

function InfoNote({ text }) {
  return (
    <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#3b82f6" strokeWidth="1.5" strokeLinecap="round" className="flex-shrink-0 mt-0.5"><circle cx="7" cy="7" r="6"/><path d="M7 6v4M7 4.5h.01"/></svg>
      <p className="text-xs text-blue-600 leading-relaxed">{text}</p>
    </div>
  )
}

function ChainIcon() { return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="white" strokeWidth="1.5" strokeLinecap="round"><path d="M6.5 9.5l-1 1a2.5 2.5 0 003.54 3.54l2.5-2.5a2.5 2.5 0 000-3.54l-.75-.75"/><path d="M9.5 6.5l1-1a2.5 2.5 0 00-3.54-3.54l-2.5 2.5a2.5 2.5 0 000 3.54l.75.75"/></svg> }
function BloodIcon() { return <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="#dc2626" strokeWidth="1.5" strokeLinecap="round"><path d="M8 2C8 2 3 7.5 3 10.5a5 5 0 0010 0C13 7.5 8 2 8 2z"/></svg> }
function RequestIcon() { return <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="#7c3aed" strokeWidth="1.5" strokeLinecap="round"><rect x="2" y="2" width="12" height="13" rx="1"/><path d="M8 5v6M5 8h6"/></svg> }

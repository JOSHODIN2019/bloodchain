...
export default function BloodBankSettings() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await bloodbankService.getProfile()
        const u   = res.user
        setForm({
  ...
  const handleSubmit = async (e) => {
    ...
    setSuccess('')
    try {
      const res = await bloodbankService.updateProfile(form)
      updateUser(res.user)
  ...
  const initials = (user?.organizationName || user?.fullName || 'BB')
  ...
  return (
    <div className="p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-0.5">Manage your blood bank profile and account details</p>
      </div>
      ...
      {/* Profile card */}
      <div className="rounded-2xl overflow-hidden bg-gradient-to-br from-emerald-600 to-teal-700 p-6 text-white">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              <span className="text-xs bg-white/20 backdrop-blur-sm px-2.5 py-1 rounded-lg font-mono">{user?.userId}</span>
              <span className="text-xs bg-white/20 backdrop-blur-sm px-2.5 py-1 rounded-lg capitalize">Blood Bank</span>
              {user?.state && (
                <span className="text-xs bg-white/20 backdrop-blur-sm px-2.5 py-1 rounded-lg">{user.state}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-neutral-200 divide-y divide-neutral-100">
        ...
        <div className="px-6 py-4">
          ...
          <p className="text-xs text-neutral-400 mt-0.5">Update your blood bank's public information</p>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full border border-neutral-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 resize-none"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="w-full border border-neutral-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 bg-white"
            >
        ...
        <div className="px-6 py-4">
          <button
            type="submit"
            disabled={saving}
            className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-colors"
          >
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 divide-y divide-neutral-100">
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          {[
            ...
            { label: 'Blood Bank ID', value: user?.userId },

...
const NAV = [
  { to: '/bloodbank',            label: 'Dashboard',      icon: DashIcon,    end: true },
  { to: '/bloodbank/donations',  label: 'Donations',      icon: DropIcon             },
  { to: '/bloodbank/inventory',  label: 'Inventory',      icon: InventoryIcon        },
  { to: '/bloodbank/requests',   label: 'Requests',       icon: RequestIcon          },
  { to: '/bloodbank/blockchain', label: 'Blockchain',     icon: ChainIcon            },
  { to: '/bloodbank/settings',   label: 'Settings',       icon: SettingsIcon         },
]
      ...
      <aside className={[
        ...
        <div className="px-5 py-5 border-b border-neutral-100">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-neutral-400 mt-0.5">Blood Bank Portal</p>
            </div>
          </div>
        </div>
        ...
        {user && (
          <div className="px-4 py-3 border-b border-neutral-100">
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5">
              <p className="text-xs font-semibold text-neutral-800 truncate">{user.organizationName || user.fullName}</p>
              <p className="text-[10px] text-emerald-600 font-mono">{user.userId}</p>
            </div>
          </div>
        )}
          ...
          {NAV.map(({ to, label, icon: Icon, end }) => (
            <NavLink
              ...
              className={({ isActive }) => [
                ...
                isActive
                  ? 'bg-emerald-600 text-white shadow-[0_2px_8px_rgb(5,150,105,0.35)]'
        ...
        <div className="px-3 pb-4 border-t border-neutral-100 pt-3 space-y-1">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-neutral-50">
            ...
            <Badge variant="success" size="sm">Bank</Badge>
          </div>
          <button
...
function InventoryIcon({ size = 16 }) {
  return <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><rect x="1" y="4" width="14" height="10" rx="1.5"/><path d="M5 4V3a1 1 0 011-1h4a1 1 0 011 1v1"/><path d="M1 8h14"/></svg>
}
function RequestIcon({ size = 16 }) {
  return <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M10 1.5H4a1.5 1.5 0 00-1.5 1.5v10A1.5 1.5 0 004 14.5h8a1.5 1.5 0 001.5-1.5V5l-3.5-3.5z"/><path d="M10 1.5V5H13.5M5.5 8.5h5M5.5 11h3"/></svg>
}
